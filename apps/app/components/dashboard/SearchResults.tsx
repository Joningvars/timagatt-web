...
export function SearchResults({
  ...
  if (!hasResults) {
    return (
      <div className="absolute top-full left-0 right-0 mt-2 rounded-lg border border-slate-100 bg-white p-4 shadow-lg">
        <p className="text-center text-xs text-slate-500">No results found.</p>
      </div>
    );
  }

  return (
    <div className="absolute top-full left-0 right-0 mt-2 max-h-[400px] overflow-y-auto rounded-lg border border-slate-100 bg-white p-2 shadow-lg z-50">
      {results.features.length > 0 && (
        <div className="mb-2">
          <div className="mb-1 px-2 py-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400">
            {t('sidebar.tracking')}
          </div>
